/**
 * The characters here were mainly taken from
 * [confusables](https://github.com/gc/confusables) by
 * [gc](https://github.com/gc), licensed under the MIT License.
 *
 * Copyright (c) 2019 gc.
 */
import { getCode } from '@skyra/char';

/**
 * A map of confusable characters, keyed by their base character.
 */
...
/**
 * A map of confusable character to base string for efficient lookup.
 *
 * @remarks
 * Only confusable characters that map to a base string containing only one
 * Unicode codepoint are included in this map (i.e. the character `ᴁ`, which
 * maps to `AE`, would not be present here).
 */
export const singleCharacterConfusables = new Map<number, number>();

/**
 * A map of confusable character to base string for efficient lookup.
 *
 * @remarks
 * Only confusable characters that map to a base string containing two Unicode
 * codepoints are included in this map (for example, the character `ᴁ`, which
 * maps to `AE`, is included here).
 */
export const doubleCharacterConfusables = new Map<number, [number, number]>();

for (const [base, aliases] of characters) {
	const isSingleCharacter = [...base].length === 1;

	// If the base string is more than 1 character in length (e.g. 'AE') we need
	// to represent it as a tuple of two characters. Base characters are always
	// either 1 or 2 characters in length.
	const key: number | [number, number] = isSingleCharacter
		? getCode(base)
		: [base.codePointAt(0)!, base.codePointAt(1)!];

	for (const alias of aliases) {
		const aliasCodePoint = getCode(alias);

		if (isSingleCharacter) singleCharacterConfusables.set(aliasCodePoint, key as number);
		else doubleCharacterConfusables.set(aliasCodePoint, key as [number, number]);
	}
}
